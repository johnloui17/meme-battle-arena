...
`(app)`-area pages are wrapped by `AuthGuard` (`src/components/guards/auth-guard.tsx`), which trusts a locally-decoded JWT `exp` rather than pinging the server to validate the session — the first real API call is what triggers the refresh flow on an actually-expired token.
...
### Widgets live outside this repo
Per `TECHSPEC.md` §9, all visual components — `MemeCard`, `VoteButton`, `BattleStage`, `LeaderboardTable`, `UploadDropzone`, plus generic `Button`/`StatPill`/`PageHeader`/`AppShell` — are developed and storied in an external shared Storybook monorepo and consumed here as the `@ntrs/core` and `@ntrs/meme` npm packages, including design tokens (`globals.css` imports `@ntrs/core/tokens.css` rather than defining its own). **Do not add new visual components under `src/`** — anything in `src/layout/**` should wire those package components to Redux/routing via props, not reimplement them. If a needed primitive is missing from `@ntrs/core`, that's a change to make in the other repo, not here.
