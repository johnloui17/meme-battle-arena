...
**Widgets: two regimes.** Pages rebuilt from the `apps/front-end/docs/UI-design/*.html` mockups (currently landing and leaderboard) are hand-built in-app with Tailwind + the `mba-*` tokens in `globals.css` (shared chrome like `SiteHeader` in `src/components/shell/`), and deliberately do not use `@ntrs` packages. Pages not yet redesigned (arena, upload, my-memes, register, login) still consume visual components from the external Storybook monorepo as `@ntrs/core`/`@ntrs/meme` per `TECHSPEC.md` §9, with `src/layout/**` only wiring them to Redux/routing.
